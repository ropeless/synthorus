...
def load_spec_file(
    ...
    def _check_module_default(_key: str, _value: Any, _alt_override: Any) -> None:
        """
        Add `_key = _value` to the `defaults_copy` dictionary
        if `_value` is not None and _key is not present.
        """
...
def interpret_spec_file(
    ...
    crosstabs: Dict[str, ModelCrosstabSpec] = _interpret_crosstabs(spec_dict, rvs, datasources)
    entities: Dict[str, ModelEntitySpec] = _interpret_entities(spec_dict, rvs.keys())
...
def _interpret_entities(spec_dict: SpecDict, all_rvs: Iterable[str]) -> Dict[str, ModelEntitySpec]:
    ...
    # Confirm foreign entities are valid
    for entity_name, entity_spec in result.items():
        if entity_spec.parent is not None and entity_spec.parent not in result:
            raise entities_dict.sub_dict(entity_name).error(f'entity has invalid parent', entity_spec.parent)
    return result
...
def _interpret_entity(entity_dict: SpecDict) -> ModelEntitySpec:
    ...
    duplicated_fields: str = ', '.join(duplicates([count_field_name, id_field_name, str(foreign_field_name)]))
    if len(duplicated_fields) > 0:
        raise entity_dict.error(f'count, id and foreign field names must be different', duplicated_fields)
